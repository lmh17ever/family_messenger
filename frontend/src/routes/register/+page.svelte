<script lang="ts">
	...
	async function submit(event: SubmitEvent) {
		...
		try {
			...
			ui.success('Аккаунт создан');
			await goto('/chats');
		} catch (cause) {
			error = cause instanceof ApiError ? cause.message : 'Не удалось зарегистрироваться';
		} finally {
			busy = false;
		}
	}
...
<main class="grid min-h-dvh place-items-center p-4">
	...
	>
		<div class="mb-6 flex flex-col items-center gap-2">
			<Logo />
			<p class="text-sm text-surface-700-300">Создайте аккаунт (до 30 символов в имени)</p>
		</div>
		...
		<label class="mb-4 block">
			<span class="mb-1 block text-xs text-surface-700-300">Повторите пароль</span>
			<input
				class="w-full rounded-base border border-surface-300-700 bg-surface-200-800 px-3 py-2 text-sm outline-none placeholder:text-surface-700-300 focus:border-primary-500"
				type="password"
				bind:value={repeat}
		...
		<p class="mt-4 text-center text-sm text-surface-700-300">
			Уже есть аккаунт?
			<a class="text-primary-500 hover:underline" href="/login">Войти</a>
		</p>
	</form>
</main>
